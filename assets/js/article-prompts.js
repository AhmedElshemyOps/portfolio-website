...
(function setupArticlePrompts() {
  ...
  buttons.forEach((button, index) => {
    ...
    const header = button.closest('.prompt-card-top, .article-prompt-header');
    const status = card?.querySelector('[data-copy-status], .copy-status, .article-copy-status');
    ...
    button.innerHTML = '<span class="copy-action-icon" aria-hidden="true">⧉</span><span>Copy prompt</span>';
    ...
    if (header && card && !header.querySelector('.prompt-expand-action')) {
      const actions = document.createElement('div');
      actions.className = 'prompt-card-actions';
      const expand = document.createElement('button');
      expand.type = 'button';
      expand.className = 'prompt-expand-action';
      expand.setAttribute('aria-expanded', 'false');
      expand.innerHTML = '<span aria-hidden="true">↕</span><span>Expand prompt</span>';
      button.replaceWith(actions);
      actions.append(expand, button);
      expand.addEventListener('click', () => {
        const expanded = card.classList.toggle('is-expanded');
        card.classList.toggle('is-collapsed', !expanded);
        expand.setAttribute('aria-expanded', String(expanded));
        expand.lastElementChild.textContent = expanded ? 'Collapse prompt' : 'Expand prompt';
        if (!expanded) card.scrollIntoView({ block: 'start', behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' });
      });
    }
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (status) status.textContent = 'The complete prompt is ready to paste. Replace placeholders and verify the output before operational use.';
        window.setTimeout(() => {
  ...
  const cards = [...new Set(buttons.map((button) => button.closest('.prompt-card, .article-prompt')).filter(Boolean))];
  if (cards.length > 1) {
    const navigator = document.createElement('nav');
    navigator.className = 'prompt-navigator';
    navigator.setAttribute('aria-label', 'Prompt template navigation');
    navigator.innerHTML = '<strong>Prompt templates</strong><span data-prompt-position>1 of 1</span><div><button type="button" data-prompt-previous>Previous</button><button type="button" data-prompt-next>Next</button></div>';
    ...
    const move = (amount) => {
      ...
      window.setTimeout(() => card.querySelector('button, a, [tabindex]')?.focus({ preventScroll: true }), 450);
      render();
    };
