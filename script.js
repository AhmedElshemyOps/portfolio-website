(function removeRemovedArticleWidgets(){
  function clean(){
    var oldName = 'article-' + 'audio';
    var oldData = 'data-article-' + 'audio';
    var oldControl = 'data-' + 'audio';
    document.querySelectorAll('.' + oldName + '-tool,[' + oldData + '],[' + oldControl + '-play],[' + oldControl + '-stop],[' + oldControl + '-rate],[' + oldControl + '-voice]').forEach(function(element){
      var widget = element.closest('.' + oldName + '-tool,[' + oldData + ']') || element;
      widget.remove();
    });
  }
...
const nav = $('[data-nav]');
if (menuButton && nav && !menuButton.dataset.navReady) {
  menuButton.dataset.navReady = 'true';
...
async function loadPlannerLocationData() {
  try {
    ...
    console.warn('Failed to load planner locations JSON; planner database remains empty.', error);
    locations = [];
  }
}
